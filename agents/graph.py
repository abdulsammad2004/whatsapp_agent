...
def showtime_node(state: CinemaAgentState):
    ...
    # Step 3: Hardened Fallback. If tool execution was skipped, intercept text generation and guide them safely
...
def booking_node(state: CinemaAgentState):
    """Transactional Node: Protects checkout flows and triggers reservation updates securely."""
    ...
    formatting_instruction = (
        ...
        "CRITICAL SAFETY RULE: If the 'reserve_seats' tool was NOT executed, it means the exact showtime code (e.g., st_101) or ticket count is missing. "
        "Do NOT invent pricing or say their booking is confirmed. Instead, ask them clearly to provide their specific showtime code.\n\n"
        "LIMITATIONS:\n"
    ...
    if ai_message.tool_calls:
        ...
        for tool_call in ai_message.tool_calls:
            ...
            if tool_name in TOOL_MAP:
                tool_output = TOOL_MAP[tool_name].invoke(tool_args)
                if tool_name == "fetch_movies" or tool_name == "fetch_showtimes":
                    next_status = "selecting_showtime"
            else:
                tool_output = f"Error: Tool '{tool_name}' not found."
    ...
    # Intercept tool-skips and ask for code input cleanly
    conversational_response = llm.invoke([SystemMessage(content=formatting_instruction)] + state["messages"])
